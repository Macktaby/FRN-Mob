angular.module('imageID.controllers').controller('DesignerCtrl', function($scope, $timeout, Restangular, $stateParams,
MemoryService, $state, $ionicSlideBoxDelegate, AuthService, $http, $ionicLoading, $ionicPopup, WORDPRESS_REST_BASE_URL) {
    $scope.data = {};
	...
	if ($scope.data.designer != null && $scope.data.designer.id == $stateParams.id) {
        $scope.renderData();
    } else {
        $ionicLoading.show({
            template: 'Loading..'
        });
        $scope.productPromise = Restangular.all("type/designer/" + $stateParams.id).getList().then(function(posts) {
            $scope.data.designer = posts[0];
            $scope.renderData();
            $ionicLoading.hide();
        }).catch(function(err) {
            $ionicLoading.hide();
